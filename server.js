...
const clientHTML = Handlebars.compile(fs.readFileSync('./client.hbs').toString());
...
app.get('/:id',  async (req, res) => {
  ...
  res.end(clientHTML({clientJS,startText}))
})
...
function idToFilePath(id){
  return './data/'+id.toString().replace(/[^a-z0-9]/gi,'')+'.dat.json'
}
  ...
  ws.on('message', async function message(data) {
    ...
    switch (parsed.action){
      case 'join-room':
        ...
        break;
      case 'set-text':
        (rooms[parsed.id]||[]).forEach(wst=>wst !== ws && wst.send(JSON.stringify(parsed)))
