...
getKey()
  .then((key)=>{
    ...
    function updateTitle(decoded){
      ...
      document.title=(decoded.split('\n').find(l=>l.trim()) || 'New document').slice(0,50)
    }
    ...
    async function setText(parsed){
      ...
      updateTitle(decoded)
      editor.style.backgroundColor='hsl('+Math.floor(Math.random()*360)+', 100%, 90%)'
    }
    if(startText){
      setText(startText)
    }
    ...
    socket.addEventListener('open', function (event) {
      ...
      editor.addEventListener('keyup',async e=>{
        try{
          ...
          send({action:'set-text', ciphertext:bufferToS(encrypted), iv:bufferToS(iv), id})
        }catch(e){
          console.error(e)
        }
      })
